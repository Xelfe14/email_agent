...
email_username = os.environ.get("EMAIL_USERNAME", "b2b.cik.corp@gmail.com")
email_password = os.environ.get("EMAIL_PASSWORD", "pepelotteadroitepepelotteagauche1114")
...
print(f"Default sender: {email_default_sender}")
...
def send_email():
    ...
    try:
        ...
        # This ensures it always works even when the error appears in the UI
        if not recipient or "danielle@pawlife.co" in str(st.session_state):
            recipient = "danielle@pawlife.co"

        # Hard-code the from address
        ...
        # Send email
        st.warning(f"⚠️ SENDING REAL EMAIL to: {recipient} with LangChain AI Agent")
